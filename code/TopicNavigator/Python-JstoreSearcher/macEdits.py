...
def createHtmlFiles(term,f):
	...
	f.write(u'<!-- File generated by Marty Dang-->' + '\n')

	#Add in standard HTML stuff
	f.write(u'<!DOCTYPE html>' + '\n')
	f.write(u'<html lang=' + '"en"'+ '>' + '\n')
	f.write(u'<meta charset="utf-8">' + '\n' + '\n')
	f.write(u'  ' + '<body>' + '\n')
	f.write(u'  ' + '<center><h2>Digital Ricoeur - JStor </h2></center>' + '\n' +'\n')
	f.write(u'  ' + '<h3>Search Term: ' + term + '</h3>' + '\n')
	f.write(u'  ' + '<h3>Book Results: ### ' + '</h3>' + '\n')
	f.write(u'  ' + '<h3>Journal Results: !!! ' + '</h3>' + '<br>' + '\n')
	f.write(u'  ' + '<p>' + '\n')
		...
		for elem in item.iter():
			...
			if(elem.tag in attirbutes):
				...
				if(elem.tag == "self-uri"):
					dirtyString = unicode(elem.attrib.values())
				...
				# changes alot
					...
					while(counter != len(parts)):
						abstract = abstract + unicode(parts[counter].text) + unicode(parts[counter].tail)
						counter += 1
...
def addBooktoHtmlFile(dictionaryOfResults, file, term,f):
    ...
    if("book-title" in dictionaryOfResults.keys()):
    	f.write("  " + "Book Title: " + unicode(dictionaryOfResults["book-title"] + "-"))

    if("book-subtitle" in dictionaryOfResults.keys()):
    	f.write(unicode(dictionaryOfResults["book-subtitle"]) + '<br>' + '\n')

    if("publisher-name" in dictionaryOfResults.keys()):
    	f.write("  " + "Publisher: " + unicode(dictionaryOfResults["publisher-name"]) + '<br>' + '\n')

    if("book-surname" in dictionaryOfResults.keys()):
    	f.write("  " + "Author(s): " + unicode(dictionaryOfResults['book-surname']))

    if("book-given-names" in dictionaryOfResults.keys()):
    	f.write(" " + unicode(dictionaryOfResults["book-given-names"]) + '<br>' + '\n')

    f.write("  Abstract: " + unicode(dictionaryOfResults["book-abstract"]) + '<br>' + '\n')

    f.write("  " + "Link: " + '<a href=' + '"' + dictionaryOfResults["self-uri"] + '"'+ 
    	'>'+ unicode(dictionaryOfResults["self-uri"])  + '</a>' + ' '+ '<br> ' + '\n')
...
def addChaptertoHtmlFile(chapterInfo, bookInfo, file, term,f):
    ...
    if("label" in chapterInfo.keys()):
    	f.write("  " + unicode(chapterInfo["label"]))

    if("title" in chapterInfo.keys()):
    	f.write("  " + unicode(chapterInfo["title"]))
    ...
    if("chapter-surname" in chapterInfo.keys()):
    	f.write("  Author(s) " + unicode(chapterInfo["chapter-surname"]))

    if("chapter-given-names" in chapterInfo.keys()):
    	f.write(" " + unicode(chapterInfo["chapter-given-names"] + '<br>' + '\n'))

    if('chapter-abstract' in chapterInfo.keys()):
    	f.write("  Abstract: " + unicode(chapterInfo['chapter-abstract']) + '<br>' + '\n')
...
def addJournalToHtmlFile(dictionaryOfResults, file, term,f):
    ...
    if("journal-title" in dictionaryOfResults.keys()):
    	f.write("Journal Title: " + unicode(dictionaryOfResults["journal-title"])
    		+ '<br>' +'\n')

    if("publisher-name" in dictionaryOfResults.keys()):
    	f.write("Publisher: " + unicode(dictionaryOfResults["publisher-name"] )
    		+ '<br>' + '\n')

    if("issue-id" in dictionaryOfResults.keys()):
    	f.write("Issue: " + unicode(dictionaryOfResults["issue-id"] )
    		+ '<br>' + '\n')

    if("month" in dictionaryOfResults.keys()):
    	f.write("Date: " + unicode(dictionaryOfResults["month"] + " "))

    if("day" in dictionaryOfResults.keys()):
    	f.write(unicode(dictionaryOfResults["day"]) + ", ")

    if("year" in dictionaryOfResults.keys()):
    	f.write(unicode(dictionaryOfResults["year"]) + '<br>' + '\n')

    if("surname" in dictionaryOfResults.keys()):
    	f.write("Author(s): " + unicode(dictionaryOfResults["surname"]))

    if("given-names" in dictionaryOfResults.keys()):
    	f.write(" " + unicode(dictionaryOfResults["given-names"] + '<br>' + '\n'))

    f.write("Abstract: " + unicode(dictionaryOfResults["abstract"] )
    	+ '<br>' + '\n')
    f.write("Link: " + '<a href=' + '"' + unicode(dictionaryOfResults["self-uri"]) + '"'+ 
    	'>'+ unicode(dictionaryOfResults["self-uri"])  + '</a>' + ' '+ '\n')

    f.write(u"<hr>" + u'<br>' + '\n' +'\n' )

# iterate through and find place holders and replace with counters
def writeCounters(term,f):
	with io.open('Digital-Ricoeur-JStor-'+ term +'.html','r') as f:
		data = f.readlines()
		print 
		data[9] = data[9].replace('###',globalBook)
		data[10] = data[10].replace('!!!',globalJournal)

	with io.open('Digital-Ricoeur-JStor-'+ term +'.html', 'w', 'utf-8') as f:
		f.writelines(data)
...
def main():
	...
	for term in terms:
		...
		print("Time to auto-gen page for " + unicode(term) + ": " + unicode(end1-start1) + " secs ")
		break
	end = time.time()

	print("Overall Time: " + unicode((end-start)/3600.00) + " hours ")
